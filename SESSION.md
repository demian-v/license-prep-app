...
| #12 | `sendEmailVerification()` never called; `emailVerified` gates nothing | ✅ **DONE** | `trial-gate.test.ts` — 4 tests + verified live (Auth emulator issued `VERIFY_EMAIL`) | Email sent at signup; trial now requires a verified address |
...
| 2026-09-16 | #12 gates the trial on a verified email, and #58 was pulled in to make that humane | Owner chose the full option. Gating without the UI work would have sent every new signup to the blank screen of #58. The server returns a machine-readable `details.reason`, so the client can say the specific true thing rather than failing silently |
