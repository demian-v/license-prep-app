...
exports.validatePurchaseReceipt = void 0;
...
const APPLE_SHARED_SECRET = (_a = functions.config().apple) === null || _a === void 0 ? void 0 : _a.shared_secret;
...
function getGoogleCredentials() {
    ...
    const configCreds = (_a = functions.config().google) === null || _a === void 0 ? void 0 : _a.credentials;
    if (!configCreds) {
...
async function retryWithBackoff(fn, maxRetries = 3, baseDelay = 1000) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (((_a = error.response) === null || _a === void 0 ? void 0 : _a.status) && nonRetryableErrors.includes(error.response.status)) {
...
async function validateAppleReceipt(receiptData, productId) {
    ...
    try {
        ...
        let isSandbox = true;
        try {
            response = await retryWithBackoff(() => axios_1.default.post(APPLE_SANDBOX_URL, requestBody, {
            ...
            if (response.data.status === 21008) {
                ...
                response = await axios_1.default.post(APPLE_PRODUCTION_URL, requestBody, {
                    headers: { 'Content-Type': 'application/json' },
                    timeout: 10000
                });
                isSandbox = false;
        ...
        const matchingReceipt = latestReceipts.find((r) => r.product_id === productId);
        if (!matchingReceipt) {
            console.error(`❌ Product ID "${productId}" not found in receipt`);
            console.error(`📋 Available products:`, latestReceipts.map((r) => r.product_id));
            return {
                valid: false,
 ...
 * subscription data including expiration dates and transaction IDs.
 */
async function validateGooglePlayReceipt(purchaseToken, productId) {
    ...
    try {
        ...
        const response = await retryWithBackoff(() => androidPublisher.purchases.subscriptionsv2.get({
 ...
 */
async function getSubscriptionMetadata(productId) {
 ...
 * Phase 4: Helper function
 */
async function findExistingSubscription(userId) {
 ...
 */
async function createOrUpdateSubscription(userId, productId, expiresAt, platform, transactionId) {
 ...
 */
exports.validatePurchaseReceipt = functions.https.onCall(async (data, context) => {
    ...
    if (!context.auth) {
        ...
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to validate receipts');
    }
    ...
    if (rateLimitExceeded) {
        ...
        throw new functions.https.HttpsError('resource-exhausted', 'Too many validation attempts. Please try again in an hour.');
    }
    // ========================================================================
    ...
    if (!receipt || !platform || !productId) {
        ...
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: receipt, platform, and productId are required');
    }
    // Validate platform
    if (platform !== 'ios' && platform !== 'android') {
        console.error(`❌ Invalid platform: ${platform}`);
        throw new functions.https.HttpsError('invalid-argument', 'Platform must be either "ios" or "android"');
    }
    // Validate product ID
    const validProductIds = Object.values(PRODUCT_IDS);
    if (!validProductIds.includes(productId)) {
        console.error(`❌ Invalid product ID: ${productId}`);
        throw new functions.https.HttpsError('invalid-argument', `Invalid product ID. Must be one of: ${validProductIds.join(', ')}`);
    }
    ...
    try {
        ...
        const subscriptionId = await createOrUpdateSubscription(userId, productId, validationResult.expiresAt, platform, validationResult.transactionId);
        ...
        throw new functions.https.HttpsError('internal', 'Failed to validate receipt. Please try again or contact support.', { originalError: error instanceof Error ? error.message : String(error) });
    }
});
// ============================================================================
